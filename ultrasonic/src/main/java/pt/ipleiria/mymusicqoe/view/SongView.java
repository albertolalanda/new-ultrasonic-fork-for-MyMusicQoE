...
public class SongView extends UpdateView implements Checkable
{
	...
	protected void setSong(final Entry song, boolean checkable, boolean draggable, boolean starable)
	{
		...
		if (viewHolder.track != null)
		{
			...
			{
				viewHolder.track.setVisibility(GONE);
			}
		}
		...
		{
			viewHolder.check.setVisibility(checkable && !song.isVideo() ? VISIBLE : GONE);
		}

		if (viewHolder.drag != null)
		{
			viewHolder.drag.setVisibility(draggable ? VISIBLE : GONE);
		}

		if (viewHolder.star != null)
		{
			viewHolder.star.setVisibility(starable ? VISIBLE : GONE);
