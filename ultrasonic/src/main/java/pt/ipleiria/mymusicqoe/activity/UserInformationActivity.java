...
public class UserInformationActivity extends SubsonicTabActivity {
	...
	private void setUserInformationRest() {
		ModalBackgroundTask<Boolean> task = new ModalBackgroundTask<Boolean>(this, false) {
			...
			protected void error(Throwable error) {
				Log.w(TAG, error.toString(), error);
				new ErrorDialog(getInstance(), String.format("%s %s", getResources().getString(R.string.settings_connection_failure), getResources().getString(R.string.user_information_error_message)), true, true);
			}
		};
		task.execute();
	}
